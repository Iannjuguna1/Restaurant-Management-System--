...
if (isset($_POST['search'])) {
    ...
    $select_orders = $conn->prepare("SELECT * FROM `orders` WHERE (email LIKE ? OR number LIKE ?) AND payment_status = 'completed'");
    $select_orders->execute(["%$search_term%", "%$search_term%"]);
} else {
    // If no search query, fetch completed orders only
...
<section class="placed-orders">
    ...
    <h1 class="heading">Placed Orders (Completed)</h1>
